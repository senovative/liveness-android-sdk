...
import io.senovative.liveness.sdk.utils.BitmapUtils.saveBitmapToFile
import io.senovative.liveness.sdk.utils.FileUtils
import io.senovative.liveness.sdk.utils.model.LivenessResult
import io.senovative.liveness.sdk.view.GraphicOverlay
...
class LivenessDetectionAnalyzer(
    ...
    private fun nextDetection() {
        currentDetectionMode()?.let { detectionMode ->
            ...
            fileUri?.let { uri ->
                detectionResults.add(LivenessResult.DetectionResult(detectionMode, uri, FileUtils(
                    context
                ).getPath(uri), startDetectionTime?.let { time -> System.currentTimeMillis()-time }))
            }
        }
